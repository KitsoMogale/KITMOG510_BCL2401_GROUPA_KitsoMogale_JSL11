...
function initializeData() {
  if (!localStorage.getItem('tasks')) {
    ...
    //store variables to localStorage for theme, 'Agile Board' logo, direction of switch
...
// console.log(elements.headerBoardName)
let activeBoard = ""
...
function filterAndDisplayTasksByBoard(boardName) {
  ...
  const filteredTasks = tasks.filter(task => task.board == boardName);// compare instead of asssign
...
//  console.log(elements.columnDivs)
  elements.columnDivs.forEach(column => {
    ...
    filteredTasks.filter(task => task.status == status).forEach(task => { //compare instead of asssign
...
function setupEventListeners() {
  ...
  elements.themeSwitch.addEventListener('change', ()=>{
    // console.log(elements.themeSwitch.checked)
    toggleTheme();
...
function toggleSidebar(show) {
  ...
  if(show){
    ...
    localStorage.setItem('showSideBar','true');
    document.querySelector('.toggle-div').style.marginTop = '270px';
  }
  else{
...
//  localStorage.getItem('light-theme') == 'enabled' ? document.body.classList.toggle('light-theme', false): document.body.classList.toggle('light-theme', true)
 ...
 if(localStorage.getItem('light-theme') == 'enabled'){
  ...
  elements.themeSwitch.setAttribute('checked','true');
